...
class Object_Manager {
    ...
    Collision_Mode edge_Collision_Mode;
    
    Collision_Mode object_Collision_Mode;
    
    Collision_Mode Option_To_Collision_Mode(Option value) { // Converts the ui collision mode checkbox Option enum-
                                                             //to a collision mode enum for improved readability.
    ...
    void update_Shapes() {
        ...
        for (Shape_Object shape : shapes) {
            ...
            if (paused == false) {
                ...
                if (object_Collision_Mode == Collision_Mode.BOUNCE || 
                    object_Collision_Mode == Collision_Mode.STOP)  collision_Detection(shape); 
    ...
    void run() {
        ...
        if (initialized == false) { // The code here runs once when the program is launched or the demo is started.
            ...
            edge_Collision_Mode = Option_To_Collision_Mode(ui.edge_Collision_Mode);
            
            object_Collision_Mode = Option_To_Collision_Mode(ui.object_Collision_Mode);
